...
public class FileUploadServiceImpl implements FileUploadService {
	...
	public String test1(MultipartFile uploadFile) throws IllegalStateException, IOException {
		...
		/*
		 * DB에 업로드되는 파일 정보를 INSERT - DB INSERT -> 파일 저장 순서로 동작 만약에 파일 저장 중 예외 발생
		...
		FileDto file = FileDto.builder().fileOriginalName(uploadFile.getOriginalFilename()).fileRename(rename)
				.filePath(testWebPath).build();
	...
	public String test3(MultipartFile uploadFile) {
		...
		try {
			...
			// transfrerTo()는 Checked Exception을 던지기 때문에
			
			// 1) throws 또는 try-catch를 무조건 작성
			
			// 2) throws 작성 시 호출하는 메서드에서
			// 	  추가 예외처리 코드를 작성해야하는 번거로움이 있음
			
			// 3) try-catch 작성시
			//  	메서드 내부에서 예외가 처리되어
			// 		메서드가 종료 시 예외가 던져지지 않아
			...
			// - @Transactional 어노테이션에 rollbackFor 속성 작성
			// rollbackFor 속성 작성안해도 롤백 처리 가능
			
			
			// 예외 강제 발생
			// - Unchecked Exception은 컴파일러가 자동으로 throw 구문을 작성해줘서
			// 예외 발생 시 호출부로 던져지게 된다.
