...
const selectCommentList = () => {
  ...
  .then(html => {
    // 매개변수 html : 타임리프가 해석되어 완성된 html 코드
    /* console.log(html); */
...
const insertComment = (parentCommentNo) => {
  ...
  data.commentContent = commentContent.value; // 작성한 댓글 내용
...
const deleteComment = (btn) => {
  ...
  fetch("/comment", {
    method : "DELETE",
    headers : {"Content-Type" : "application/json"}, // 415 error
    body : commentNo
  })
  .then(response => {
    if(response.ok) return response.text();
...
addComment.addEventListener("click", () => {
  ...
  // 1) 로그인 여부 검사(boardDeatil.html의 loginCheck 전역변수)
  if(loginCheck == false) {
    alert("로그인 후 이용해주세요.");
    return;
  }
