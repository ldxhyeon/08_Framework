...
memberEmail.addEventListener("input", e => {
  ...
  if( regEx.test(inputEmail) === false ) {
    ...
    emailMessage.classList.remove("confirm"); // 빨간 글씨 추가
    return;
  }
  ...
  .then(count => {
    ...
    if(count == 1){ // 중복인 경우
      ...
      emailMessage.classList.remmove("confirm");
      checkObj.memberEmail = false;
      return;
    }
...
if(inputNickname.length === 0) {
  ...
  if( regEx.test(inputNickname) === false ) {
    ...
    nickMessage.classList.remove("confirm"); // 빨간 글씨 추가
    checkObj.memberNickname = false;
    return;
  }
  ...
  .then(count => {
    ...
    if(count == 1){ // 중복인 경우
      ...
      nickMessage.classList.remmove("confirm");
      checkObj.memberNickname = false;
      return;
    }
...
sendAuthKeyBtn.addEventListener("click", () => {
  ...
  // 1초가 지날때 마다 함수 내부 내용이 실행되는 setInterval 작성
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  // JSON.stringfy(객체) : 객체 -> JSON 변환(문자열화)
  ...
  for(let key in checkObj) { 
    ...
    if( checkObj[key] === false) { // 유효하지 않은 경우
      ...
      switch(key) {
        ...
        case "memberPwConfrim"  : str = "비밀번호 확인이 일치하지 않습니다."; break;
