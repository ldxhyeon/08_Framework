...
const connectSse = () => {
  ...
  /* 서버로 부터 전달 받은 메시지가 왔을 경우(전달 받은 경우) */
  eventSource.addEventListener("message", e => {
    console.log(e.data); // e.data : 전달 받은 메시지
                         // -> Spring HttpMessageConverter가
                         // JSON으로 변환해서 응답한다.

    const obj = JSON.parse(e.data); // 알림을 받는 사람 번호, 읽지 않은 알림 개수
    console.log(obj);
...
- 알림을 받을 특정 클라이언트의 id(memberNo 또는 memberNo를 알아낼 수 있는 값)
