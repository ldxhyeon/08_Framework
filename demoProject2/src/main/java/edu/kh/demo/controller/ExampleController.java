...
public class ExampleController {
	...
	// Servlet/JSP 내장 객체 4종류의 데이터 유지 범위
	
	// 1) page scope : 해당 페이지
	
	// 2) request scope : 요청 페이지 + 요청 위임(forward) 받은 곳
	
	// 3) session scope : 클라리언트가 서버 최초 연결 시 생성,
	//																 연결한 브라우저 종료 또는 세션 시간 만료까지
	
	// 4) application scope : 서버 실행 시 1개만 생성
	//											  서버가 종료될 때 까지 유지
	...
	public String ex4(Model model) {
		...
		Student std = new Student("33333", "먕구", 5, null);
