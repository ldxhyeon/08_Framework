...
// 3) Map<String, Object>
	...
	public String test3(
		...
		log.debug("fruitsList : {} ", fruitsList);
	...
	public String test4(
		 ...
		 * 값을 작성하지 않은 경우 빈칸"()"이 제출된!
		 * -> 제출되는 값이 존재하기 때문에
		 * 	@RequestParam() 으로 아무 속성을 적지 않아도 값을 얻어올 수 있다.
		 * 
		 * 
		 ...
		 * 
		 * @ 직접 쿼리스트링을 작성하는데
		 * 	key=values가 누락된 경우 -> 400 에러
		 * 
		 * * required = false일 때, 경우 값이 제출되지 않으면 null
		 * 
		 * * defaultValue : 
...
/* 5. @ModelAttribute 를 이용한 파라미터 얻어오기  */
	...
	public String test4(
				...
				@RequestParam(value = "memberAge") int memberAge,
				// -> key값과 변수명이 같을 경우 @RequestParam의 () 생략이 가능했지만
				// 지금은 생략하면 에러 발생
				
				// 자동으로 필드를 찾아 값을 set 한다.
