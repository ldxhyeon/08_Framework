...
const getAirQuality = async (cityName) => {
  ...
  /* 
    ...
    - async가 포함된 함수에서
      명시적으로 Prommise 객체 반환 구문을 작성하지 않을 경우
  ...
  */
  ...
  // /getServiceKey 요청에 대한 응답이 돌아올 때 까지 대기(awiat)
  ...
  // fetch API(AJAX)를 이용해서 공공데이터 비동기로 조회 -> 동기식으로 변경
  ...
  try {
    ...
    console.log(`미세먼지 등급 : ${item['pm25Grade']}`);
    ...
    // 이모지/등급 배열
    const greadEmoji = ['😄', '🙂', '😷', '🤢'];
    const greadText = ['좋음', '보통', '나쁨', '매우나쁨'];

    pm10Grade.innerText = greadEmoji[item.pm10Grade - 1];
    pm10GradeText.innerText = greadText[item.pm10Grade - 1];
    pm10Value.innerText = `미세먼지 농도 : ${item['pm10Value']} ㎍/㎥`;

    pm25Grade.innerText = greadEmoji[item.pm25Grade - 1];
    pm25GradeText.innerText = greadText[item.pm25Grade - 1];
